...
export const SettingsBar: FC<SettingsBarProps> = ({ isOpen, settings, onUpdateSettings, onToggleCustom, customActive = false, testLocked = false }) => {
  ...
  const pillClass = (active: boolean) => `px-2.5 py-1 rounded text-xs font-medium transition-all active:scale-90 ${active ? 'bg-accent text-black font-bold shadow-sm' : 'text-mutedtext hover:text-bodytext hover:bg-darkborder'}`;
  const toggleClass = (active: boolean) => `p-1.5 rounded-lg border text-xs font-medium flex items-center gap-1 transition-all glow-ring active:scale-90 ${active ? 'bg-accentmuted border-accent text-accent' : 'bg-darkbg border-darkborder text-mutedtext hover:text-bodytext'}`;
  ...
  return (
    <div className="w-full bg-darkcard border-y border-darkborder py-3.5 px-4 mb-5 shadow-inner animate-slide-down">
      <div className="max-w-6xl mx-auto flex flex-col gap-3.5 text-xs text-bodytext">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 stagger-item" style={{ '--stagger-i': 1 } as CSSProperties}>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <input type="range" min={0} max={100} step={5} value={Math.round(settings.keyboardVolume * 100)} onChange={handleVolumeChange} style={{ accentColor: 'var(--color-accent)' }} className="w-24 cursor-pointer transition-all hover:scale-105" aria-label="Keyboard sounds volume" title="Keyboard sounds volume" />
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 pt-1 stagger-item" style={{ '--stagger-i': 2 } as CSSProperties}>
          <div className="flex items-center gap-2 flex-wrap">
