...
export const LanguageModal: FC<LanguageModalProps> = ({ isOpen, onClose, currentLanguage, onSelectLanguage }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 select-none animate-fade-in">
      ...
      <div className="relative w-full max-w-lg bg-darkcard border-2 border-darkborder rounded-2xl p-6 shadow-2xl z-10 text-bodytext">
        <div className="flex items-center justify-between pb-4 border-b border-darkborder">
          ...
          <button onClick={onClose} className="p-1.5 rounded-lg bg-darkbg hover:bg-darkborder text-mutedtext hover:text-bodytext"><X className="w-4 h-4" /></button>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 mt-5 max-h-[60vh] overflow-y-auto pr-1">
          {languages.map((lang) => {
            const isSelected = currentLanguage === lang.id;
            return (
              <button key={lang.id} onClick={() => { onSelectLanguage(lang.id); onClose(); }} className={`p-3 rounded-xl border text-left transition-all flex items-center justify-between ${isSelected ? 'bg-darkbg border-accent text-accent shadow-md' : 'bg-darkbg border-darkborder hover:border-mutedtext text-bodytext'}`}>
        ...
        <div className="pt-4 border-t border-darkborder mt-5 flex items-center justify-between text-xs text-mutedtext">
          ...
          <button onClick={onClose} className="px-4 py-1.5 bg-accent text-black font-bold rounded-lg hover:bg-accenthover">Done</button>
        </div>
      </div>
    </div>
  );
